...
for fname in fnames:
    ...
    pts = np.float32([ [0,0],
                       ...
                       [0,I1.shape[0]] ]).reshape(-1,1,2) # this needs to be changed
    
    dst = cv2.perspectiveTransform(pts,H).reshape(4,2)
   
    J = I2.copy()
    cv2.line(J, (dst[0,0].astype(int), dst[0,1].astype(int)), (dst[1,0].astype(int), dst[1,1].astype(int)), (255,0,0),3)
    cv2.line(J, (dst[1,0].astype(int), dst[1,1].astype(int)), (dst[2,0].astype(int), dst[2,1].astype(int)), (255,0,0),3)
    cv2.line(J, (dst[2,0].astype(int), dst[2,1].astype(int)), (dst[3,0].astype(int), dst[3,1].astype(int)), (255,0,0),3)
    cv2.line(J, (dst[3,0].astype(int), dst[3,1].astype(int)), (dst[0,0].astype(int), dst[0,1].astype(int)), (255,0,0),3)
